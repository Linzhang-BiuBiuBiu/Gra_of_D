...
adj_log_glm <- function(data=data,
    ...
    outfile_name <- paste0(group_index,"_",length(signle_list),"_",length(adj_immo),"_.docx")
    glm_list <- list()
...
adj_cox_reg <- function(data=data,
                        ...
                        adj_immo=adj_immo,
                        live_time=live_time){
    outfile_name <- paste0(live_time,"_",length(signle_list),"_",length(adj_immo),"_cox.docx")
    ...
    if(is.null(adj_immo)){
        for(i in names(signle_list)){
            ...
            formul_index <- formula(paste0("Surv(",live_time,",",statue,")","~."))
...
#多数据的批量的多/单因素逻辑回归（比如区分age>60或者<60），
#data是data(需要先进行numeric)，signle_list是需要批量归一化的变量列表，
#adj_immo固定校正的变量，比如年龄和性别，group_index是分组变量（为1或者0），
#最后输出的是一个word 的文件，以及tbl的list,data_split是分组变量的名字
#批量logistic函数
adj_log_glm_split <- function(data=data,
                        signle_list=signle_list,
                        adj_immo=adj_immo,
                        data_split=data_split,
                        group_index=group_index){
    
    split_index <- unique(data[,data_split])
    #如果是空的，就是单因素，如果不空，就是多因素
    for(j in split_index){
        data_tem <- data[data[,data_split] %in% j,]
        
        outfile_name <- paste0(group_index,"_",data_split,"_",
                               length(signle_list),"_",length(adj_immo),
                               "_",j,"_.docx")
        ...
        if(is.null(adj_immo)){
            for(i in names(signle_list)){
                tem_name_index <- c(i,group_index)
                formul_index <- formula(paste0(group_index,"~."))
                ad_mol <- glm(formul_index,data_tem[,tem_name_index],family=binomial)
                tbl_glm_result <- tbl_regression(ad_mol,exponentiate = T)
                glm_list_tem[[i]] <- tbl_glm_result} 
                
            
           tbl_merge(tbls = glm_list_tem,tab_spanner = names(glm_list_tem)) |> 
               as_flex_table() |> 
                save_as_docx(path = outfile_name)
...
#将相同的药物名称统一，如果是一类的，任意一个出现一次都是1，否则为0
same_drup_count <- function(data=data,
                            single_drug=single_drug){
    
    single_drug <- unique(single_drug)
    data <- as.data.frame(data)
    tem_data <- data[,-(which(colnames(data) %in% single_drug))]
    
    i <- single_drug[3]
    for (i in single_drug) {
        sing_data <- data[,which(colnames(data)==i)] |> as.data.frame()
        sing_data <- apply(sing_data, 2, as.numeric)
        singl_sum <- ifelse(apply(sing_data, 1, sum)>0,1,0) |> as.data.frame()
        
        colnames(singl_sum) <- i
        
        tem_data <- cbind(tem_data,singl_sum)
    }
    return(tem_data)
}
...
#vio_box_jit_mulGrou画图函数，融合了小提琴图，箱型图，散点图以及组间分析
#data是原始数据，group是分组，target是需要关注得连续型变量得名字，step是作图y轴步长
vio_box_jit_mulGrou <- function(data=data,
                                group=group,
                                target=target,
                                step=step,
                                size=size){
    ...
    p <- ggplot(data, aes(x=data[,group], y=data[,target],color=data[,group])) +
        # 绘制箱线图
    ...
    p
    ggsave(paste0(group,"_",ckkk,"no_dif.png"),width = 4.5,height = 6)
    
    #设置组间比较
    group_uniq <- as.vector(unique(data[,group]))
    
    group_list <- list()
    for(i in group_uniq){
        
        group_uniq <- drop_vector(group_uniq,i)
        
        for(j in group_uniq){
            name_ij <- paste(i,"vs",j)
            
            list_tem <- list(c(i,j))
            group_list <- append(list_tem,group_list)
            
        }
    }
    
    y_max <- max(data[,target])
    
    y_max_con <- c(y_max+c(1:length(group_list)*step))
    
    p3 <- p + stat_compare_means(comparisons=group_list,
                                 label.y = y_max_con,
                                 method="wilcox.test",
                                 label="p.signif",
                                 family = "serif")
    ...
    p3
    ggsave(paste0(group,"_",ckkk,"diff.png"),width = 4.5,height = 6)

    
    p+p3
    ggsave(paste0(group,"_",ckkk,"both.png"),width = 9,height = 6)
}
