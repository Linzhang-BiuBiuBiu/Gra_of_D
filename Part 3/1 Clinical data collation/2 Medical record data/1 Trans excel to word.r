...
for(k in file_name){
  setwd(dirname(rstudioapi::getActiveDocumentContext()$path))
  rx <- read.xlsx(k,1,detectDates=T)
  ...
  dur_data <- gsub("初步诊断","入院诊断",dur_data)
  my_doc<-read_docx()
  my_doc<-my_doc %>% body_add_par(dur_data, style = "Normal")

  #步骤五：输出文档到本地
  setwd("D:\\onedrive\\桌面\\Clinical_data_summary\\入院+出院+手术-v2\\住院")
  filnames <- paste0(gsub("入院出院手术.xlsx","住院",k),".docx")
