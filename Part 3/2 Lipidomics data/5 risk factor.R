...
colnames(lipid_clin_coef2)[colnames(lipid_clin_coef2)%in% "Coef"] <- "Lipid Score"

colnames(lipid_clin_coef2)[10:29] <- c("DCER(24:1)","DCER(22:0)","HCER(12:0)","DCER(24:0)","PE(P-18:0/20:4)","HCER(24:1)","PE(O-18:0/20:4)","DCER(16:0)","PE(16:0/18:2)","PE(O-18:0/22:5)","PE(P-18:0/22:5)","PE(P-18:0/22:4)","PE(O-18:0/22:4)","PE(P-18:2/18:2)","PE(18:2/18:2)","HCER(16:0)","PE(16:0/20:0)","PE(P-18:0/22:6)","Lipid Score","EFI")

#作图
vio_box_jit_mulGrou(data=lipid_clin_coef2,
                    group = "分组",
                    target = "Lipid Score",
                    step=1.5,
                    size=20)

lipid_list <- name_list(c("DCER(24:1)","DCER(22:0)","HCER(12:0)","DCER(24:0)","PE(P-18:0/20:4)","HCER(24:1)","PE(O-18:0/20:4)","DCER(16:0)","PE(16:0/18:2)","PE(O-18:0/22:5)","PE(P-18:0/22:5)","PE(P-18:0/22:4)","PE(O-18:0/22:4)","PE(P-18:2/18:2)","PE(18:2/18:2)","HCER(16:0)","PE(16:0/20:0)","PE(P-18:0/22:6)","Lipid Score","EFI"))

lapply(lipid_list, function(x) vio_box_jit_mulGrou(data=lipid_clin_coef2,
                                       group = "分组",
                                       target = x,
                                       step=1.5,
                                       size=20))
...
tbl_summary(lipid_clin_coef2,by = "分组",
    ...
    add_p()|> 
    as_flex_table() |> 
    save_as_docx(path = "HF-Con-A-B-C-D_P.docx")
...
cor_lipd_data <- read.xlsx("lipid_clin_coef2.xlsx",1)
#colnames(cor_lipd_data) <- cor_lipd_data[1,]
#cor_lipd_data <- cor_lipd_data[-1,]
cor_lipd_data <- apply(cor_lipd_data[,c(-1:-9)],2,as.numeric)

cor_lab_name <- c("coef","DCER24_1", "HexCerd18_1_12_0","HSW", "BNP", "红细胞分布宽度CV","P红细胞压积", 
  "P血红蛋白","P红细胞","P白蛋白溴甲酚绿法","平均血红蛋白浓度")

cor_lipd_data1 <- cor_lipd_data[,cor_lab_name]
colnames(cor_lipd_data1) <- c("Lipid Score","DCER(24:1)","HCER(12:0)","EFI","BNP",
                              "RDW","PCT","Hb","RBC","ALB","MCHC")


cor_lipid_name <- c("DCER24_1", "DCER22_0","DCER24_0","DCER16_0", "HexCerd18_1_12_0","HCER16_0","HCER24_1", "PEP_18_0_20_4", "PEO_18_0_20_4", "PE16_0_18_2", "PEO_18_0_22_5", "PEP_18_0_22_5", "PEP_18_0_22_4", "PEO_18_0_22_4", "PEP_18_2_18_2", "PE18_2_18_2","PE16_0_20_2", "PEP_18_0_22_6")
cor_lipd_data2 <- cor_lipd_data[,cor_lipid_name]
colnames(cor_lipd_data2) <- c("DCER(24:1)", "DCER(22:0)", "DCER(24:0)", "DCER(16:0)", "HCER(12:0)", "HCER(16:0)", "HCER(24:1)", "PE(P-18:0/20:4)", "PE(O-18:0/20:4)", "PE(16:0/18:2)", "PE(O-18:0/22:5)", "PE(P-18:0/22:5)", "PE(P-18:0/22:4)", "PE(O-18:0/22:4)", "PE(P-18:2/18:2)", "PE(18:2/18:2)", "PE(16:0/20:2)", "PE(P-18:0/22:6)")

cor_lipid_result1 <- cor(cor_lipd_data1)
pmtcars1 <- cor_pmat(cor_lipid_result1)

cor_lipid_result2 <- cor(cor_lipd_data2)
pmtcars2 <- cor_pmat(cor_lipid_result2)

ggcorrplot(cor_lipid)
ggcorrplot(cor_lipid,method = "circle",lab=T)


tiff("cor_result_3.tiff",width = 2000,height = 2000,res = 200,family = "Times")
ggcorrplot(cor_lipid_result1,
           hc.order = T,  
           method = "circle",#分等级聚类重排矩阵
...
pdf("cor_result_4.pdf",width = 12,height = 12,family = "Times")
ggcorrplot(cor_lipid_result2,
           hc.order = T,  
           method = "circle",#分等级聚类重排矩阵
           ggtheme = ggplot2::theme_void(base_size = 10), #主题修改
           colors = c("CornflowerBlue","white","Salmon"), #自定义颜色，看自己喜欢，或是参考好看的文献Figure用法。
           lab = T,lab_size = 5,    #相关系数文本字体大小
           tl.cex = 20,             #坐标轴字体大小
           p.mat = pmtcars2,         #添加显著性信息
           sig.level = 0.01,        #显著性水平
           pch = 5,                 #不够显著的色块进行标记，pch表示选择不同的标记方法，可以尝试其他数字表示什么标记方法
           pch.cex = 10)            #不显著标记的大小，使用insig = "blank"将不显著的空白处理
dev.off()

lpid_cor_mod <- read.xlsx("lipid_clin_coef3.xlsx")

colnames(lpid_cor_mod) <- c("DCER(24:1)", "HCER(12:0)", "EFI", "BNP", "RBC", "Hb", 
                            "HCT", "MCHC", "RDW", "ALB")

cor_lipid_result3 <- cor(lpid_cor_mod)
pmtcars3 <- cor_pmat(cor_lipid_result3)

pdf("cor_result_5.pdf",width = 9,height = 9,family = "Times")
ggcorrplot(cor_lipid_result3,
           hc.order = T,  
           method = "circle",#分等级聚类重排矩阵
           ggtheme = ggplot2::theme_void(base_size = 10), #主题修改
           colors = c("CornflowerBlue","white","Salmon"), #自定义颜色，看自己喜欢，或是参考好看的文献Figure用法。
